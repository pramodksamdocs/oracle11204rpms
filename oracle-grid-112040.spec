...
%define home /opt/app/oracle/oragrid/11.2.0.4
%define LOG_FILE /var/opt/MFSoragrid11204.log
%define oracle_stage /opt/app/oracle/oraclegrid
%define RPM_BUILD_DIR /root/rpmbuild/BUILD
Name: oracle-grid-112040
Version: 0.01
Release: 1.el7
Summary: Oracle grid Database 11.2.0.4 in a RPM
Vendor: Oracle Corp.
Group: Applications/Databases
License: Proprietary
URL: https://www.amdocs.com/
Source0: p13390677_112040_Linux-x86-64_3of7.zip
Prefix: %{base}
AutoReqProv: no
...
%setup -n grid
mkdir -p /var/opt/gridinstall/
cp -rpf %{RPM_BUILD_DIR}/grid /var/opt/gridinstall/
chown -R oracle:dba /var/opt/gridinstall/*
exit
...
if [ $? -gt 0 ] ; then
  echo >> /var/opt/MFSoragrid11204.log
  echo "Creating oracle unix account" >> /var/opt/MFSoragrid11204.log
  # create user accounts and prompt for password
  mkdir -p /opt/app/oragrid/11.2.0.4
  mkdir -p /opt/app/oracle
  groupadd -g 501 dba
  groupadd -g 500 oinstall
  useradd -g dba -G oinstall -d /opt/app/oracle -s /bin/bash -p $(echo mypasswd | openssl passwd -1 -stdin) oracle
  chown -R oracle:dba /opt/app/oragrid
else
  echo >> /var/opt/MFSoragrid11204.log
  echo "User oracle not created, it already exists" >> /var/opt/MFSoragrid11204.log
fi
...
/usr/bin/echo "Installing  the oracle Clusterware 11.2.0.4.0 home now to $ORACLE_HOME";
/usr/bin/mkdir -p $ORACLE_HOME;

su - oracle -c "/var/opt/gridinstall/grid/runInstaller -silent -waitforcompletion -responseFile /stage/grid_install.rsp -force" >> /var/opt/MFSoragrid11204.log;
